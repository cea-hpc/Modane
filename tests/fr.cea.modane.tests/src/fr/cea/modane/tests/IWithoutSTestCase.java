...
public class IWithoutSTestCase extends TestCaseBase
{
	private final static String Dir = "IWithoutS";
	...
	public void testCompilation()
	{
		...
		assertEquals(0, launchCommand("./Hydro.exe -arcane_opt max_iteration 1 ./IWithoutS.arc", Dir, Dir));
	}
	...
	protected void makeFullClean()
	{
		for (File f : getAllFiles(Dir))
		{
			if (f.getName().matches("__(.*).h")
					...
					|| f.getName().matches("MotherDefault.(.*)")
					)
				f.delete();
		}
	}
